<?php  
	...
	$st = $cn->prepare("SELECT id_comprobante, nombre_merc_c, cantidad_cajas, precio_unitario, precio_venta FROM comprobantes c INNER JOIN numerosdm n ON c.id_dm=n.id_dm WHERE numero_dm = ? ORDER BY id_comprobante ASC");
	...
	$st = $cn->prepare("SELECT SUM(elc_m) AS sum1 FROM comprobantes c INNER JOIN numerosdm n ON c.id_dm=n.id_dm WHERE numero_dm = ?");
	$st->bindParam(1, $_GET['dm']);
	$st->execute();
	$res = $st->fetch();
	$st = $cn->prepare("SELECT SUM(alicuota_m) AS sum2 FROM comprobantes c INNER JOIN numerosdm n ON c.id_dm=n.id_dm WHERE numero_dm = ?");
	$st->bindParam(1, $_GET['dm']);
	$st->execute();
	$res1 = $st->fetch();
	$st = $cn->prepare("SELECT SUM(dai_m) AS sum3 FROM comprobantes c INNER JOIN numerosdm n ON c.id_dm=n.id_dm WHERE numero_dm = ?");
	$st->bindParam(1, $_GET['dm']);
	$st->execute();
	$res2 = $st->fetch();
	$st = $cn->prepare("SELECT cif FROM numerosdm WHERE numero_dm = ?");
	$st->bindParam(1, $_GET['dm']);
	$st->execute();
	$res3 = $st->fetch();
