<?php  
	...
	$report->Cell(50, 10, '', 0);
	$report->Cell(90, 20, utf8_decode('Reporte Tipos de usuario'), 0);
	$report->Ln(20);
	...
	$report->Cell(45, 8, utf8_decode('Código'), 1, 0, 'C');
	$report->Cell(45, 8, utf8_decode('Nombre'), 1, 0, 'C');
	$report->Cell(45, 8, utf8_decode('Descripción'), 1, 0, 'C');
	$report->Cell(45, 8, utf8_decode('Cargo'), 1, 0, 'C');
	$report->Ln(8);
	$report->SetFont('Josefin Sans', '', 10);
	$st = $cn->prepare("SELECT id_tipo_usuario, nombre_tipo_usuario, identificador_tipo_usuario, descripcion_tipo_usuario FROM tipos_usuarios ORDER BY id_tipo_usuario ASC");
	$st->execute();
	$resultado = $st->fetchAll();
	foreach ($resultado as $key => $value) {
		$report->Cell(45, 8, html_entity_decode($value['id_tipo_usuario']), 1, 0, 'C');
		$report->Cell(45, 8, html_entity_decode($value['nombre_tipo_usuario']), 1, 0, 'C');
		$report->Cell(45, 8, html_entity_decode($value['descripcion_tipo_usuario']), 1, 0, 'C');
		$report->Cell(45, 8, html_entity_decode($value['identificador_tipo_usuario']), 1, 0, 'C');
		$report->Ln(8);
	}
	$report->Ln(8);
