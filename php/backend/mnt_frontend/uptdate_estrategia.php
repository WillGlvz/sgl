	...
	<div class="container">
		<div class="panel panel-success" style="margin-top:80px;">
		  <div class="panel-heading">
		  	<p class="letra4x text-center" style="margin-top:10px;"><span class="icon-users" style="margin-right:20px;"></span>Mision empresarial</p>
		  </div>
		  <div class="panel-body">
		    ...
		    <form method="post" enctype="multipart/form-data">
			    <div class="row">
					...
					<div class="col-md-6">
			    		<img src="http://localhost/SGL/img/<?php echo $result['img_seccion']; ?>"  width="400" height="300">
...
<?php
	if (isset($_POST['btnmodi'])) {
		...
		$img = "mision.jpg";
		if ($nombre == "") {
			$st = $cn->prepare("UPDATE secciones SET titulo_seccion = ?, descripcion = ?, img_seccion = ? WHERE id_seccion = 2");
			$st->bindParam(1, $titulo);
			$st->bindParam(2, $descripcion);
			$st->bindParam(3, $img);
			if ($st->execute()) {
				echo "<script>swal('¡Completado!', 'Datos modificados con exito.', 'success');</script>";
				echo "<script>setTimeout(function(){ window.location='http://localhost/SGL/admin/cpanel/frontend-update-mision'; }, 2000);</script>";
			}else{
