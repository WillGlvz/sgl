	...
	<div class="container">
		<div class="panel panel-success" style="margin-top:80px;">
		  <div class="panel-heading">
		  	<p class="letra4x text-center" style="margin-top:10px;"><span class="icon-users" style="margin-right:20px;"></span>Vision empresarial</p>
		  </div>
...
<?php
	if (isset($_POST['btnmodi'])) {
		...
		$img = "vision.jpg";
		if ($nombre == "") {
			$st = $cn->prepare("UPDATE secciones SET titulo_seccion = ?, descripcion = ?, img_seccion = ? WHERE id_seccion = 3");
			$st->bindParam(1, $titulo);
			$st->bindParam(2, $descripcion);
			$st->bindParam(3, $img);
			if ($st->execute()) {
				echo "<script>swal('¡Completado!', 'Datos modificados con exito.', 'success');</script>";
				echo "<script>setTimeout(function(){ window.location='http://localhost/SGL/admin/cpanel/frontend-update-vision'; }, 2000);</script>";
			}else{
