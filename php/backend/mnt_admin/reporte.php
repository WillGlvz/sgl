<?php  
	...
	$report->Cell(90, 10, '', 0);
	$report->Cell(90, 20, utf8_decode('Reporte de administradores'), 0);
	$report->Ln(20);
	...
	$report->Cell(43, 8, utf8_decode('Nombres'), 1, 0, 'C');
	$report->Cell(43, 8, utf8_decode('Apellidos'), 1, 0, 'C');
	$report->Cell(43, 8, utf8_decode('Usuario'), 1, 0, 'C');
	$report->Cell(43, 8, utf8_decode('Correo'), 1, 0, 'C');
	$report->Cell(43, 8, utf8_decode('Tipo usuario'), 1, 0, 'C');
	$report->Ln(8);
	$report->SetFont('Josefin Sans', '', 10);
	$st = $cn->prepare("SELECT id_admin, nombres_admin, apellidos_admin, usuario_admin, correo_admin, nombre_tipo_usuario FROM administradores a INNER JOIN tipos_usuarios t ON a.id_tipo_usuario=t.id_tipo_usuario ORDER BY id_admin ASC");
	$st->execute();
	$resultado = $st->fetchAll();
	foreach ($resultado as $key => $value) {
		$report->Cell(43, 8, html_entity_decode($value['id_admin']), 1, 0, 'C');
		$report->Cell(43, 8, html_entity_decode($value['nombres_admin']), 1, 0, 'C');
		$report->Cell(43, 8, html_entity_decode($value['apellidos_admin']), 1, 0, 'C');
		$report->Cell(43, 8, html_entity_decode($value['usuario_admin']), 1, 0, 'C');
		$report->Cell(43, 8, html_entity_decode($value['correo_admin']), 1, 0, 'C');
		$report->Cell(43, 8, html_entity_decode($value['nombre_tipo_usuario']), 1, 0, 'C');
		$report->Ln(8);
	}
	$report->Ln(8);
